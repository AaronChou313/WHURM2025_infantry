...
/**
 * @brief 初始化gimbal_control变量，包括pid初始化、遥控器指针初始化、云台电机指针初始化、陀螺仪角度指针初始化
 * @param[out] init gimbal_control变量指针
 * @retval none
 */
static void gimbal_init(gimbal_control_t *init);

/**
 * @brief 【新版】初始化gimbal_control变量，包括pid初始化、遥控器指针初始化、云台电机指针初始化、陀螺仪角度指针初始化
 * @param[out] init gimbal_control变量指针
 * @retval none
 */
static void gimbal_init_1(gimbal_control_t *init);

/**
 * @brief 设置云台控制模式，主要调用gimbal_behaviour_mode_set函数执行具体功能
 * @param[out] gimbal_set_mode gimbal_control变量指针
 * @retval none
 */
 ...
 * @param[out] gimbal_feedback_update gimbal_control变量指针
 * @retval none
 */
 ...
 * @param[out] mode_change gimbal_control变量指针
 * @retval none
 */
...
/**
 * @brief 设置云台控制设定值，主要通过调用gimbal_behaviour_control_set函数执行具体功能
 * @param[out] gimbal_set_control gimbal_control变量指针
 * @retval none
 */
 ...
 * @param[out] gimbal_control_loop gimbal_control变量指针
 * @retval none
 */
static void gimbal_control_loop(gimbal_control_t *control_loop);

/**
 * @brief 云台控制模式为GIMBAL_MOTOR_GYRO，使用陀螺仪计算的欧拉角进行控制
 * @param[out] gimbal_motor yaw电机或pitch电机
 * @retval none
 */
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor);

/**
 * @brief 云台控制模式为GIMBAL_MOTOR_ENCONDE，使用编码器计算的相对角度进行控制
 * @param[out] gimbal_motor yaw电机或pitch电机
 * @retval none
 */
...
/**
 * @brief 云台控制模式为GIMBAL_MOTOR_RAW，电机直接发送到CAN总线
 * @param[out] gimbal_motor yaw电机或pitch电机
 * @retval none
 */
...
/**
 * @brief 在GIMBAL_MOTOR_GYRO模式下，限制角度，避免超出最大角度
 * @param[out] gimbal_motor yaw电机或pitch电机
 * @retval none
 */
...
/**
 * @brief 【新版】在GIMBAL_MOTOR_GYRO模式下，限制角度，避免超出最大角度
 * @param[out] gimbal_motor yaw电机或pitch电机
 * @retval none
 */
static void gimbal_absolute_angle_limit_1(gimbal_motor_t *gimbal_motor, fp32 add);

/**
 * @brief 在GIMBAL_MOTOR_ENCODE模式下，限制角度，避免超出最大角度
 * @param[out] gimbal_motor yaw电机或pitch电机
 * @retval none
 */
...
/**
 * @brief 【新版】在GIMBAL_MOTOR_ENCODE模式下，限制角度，避免超出最大角度
 * @param[out] gimbal_motor yaw电机或pitch电机
 * @retval none
 */
static void gimbal_relative_angle_limit_1(gimbal_motor_t *gimbal_motor, fp32 add);
...
/**
 * @brief 云台PID清除，清除pid的out和iout
 * @param[out] pid_clear gimbal_control变量指针
 * @retval none
 */
 ...
 * @param[out] yaw_offset yaw电机中值
 * @param[out] pitch_offset pitch电机中值
 * @param[out] max_yaw yaw电机最大机械角度
 * @param[out] min_yaw yaw电机最小机械角度
 * @param[out] max_pitch pitch电机最大机械角度
 * @param[out] min_pitch pitch电机最小机械角度
 * @retval none
 */
...
{
  ReadData = (KEY_1) ^ 0xff;
  keytrg = ReadData & (keycon ^ ReadData);
  keycon = ReadData;
}
...
void gimbal_task(void const *pvParameters)
{
  ...
  gimbal_init_1(&gimbal_control_1);
  // 射击初始化
  shoot_init();
  while (1)
  {
    ...
    data_transition();
		USART1_SEND();
    gimbal_set_mode(&gimbal_control_1);                    // 设置云台控制模式
    gimbal_mode_change_control_transit(&gimbal_control_1); // 控制模式切换后，控制数据过渡
    gimbal_feedback_update(&gimbal_control_1);             // 云台数据反馈
    gimbal_set_control(&gimbal_control_1);                 // 设置云台控制量
    gimbal_control_loop(&gimbal_control_1);                // 云台控制PID计算
    shoot_can_set_current = shoot_control_loop();          // 射击任务控制循环
#if YAW_TURN
    yaw_can_set_current = -gimbal_control_1.gimbal_yaw_motor.given_current;
#else
    yaw_can_set_current = gimbal_control_1.gimbal_yaw_motor.given_current;
#endif

#if PITCH_TURN
    pitch_can_set_current = -gimbal_control_1.gimbal_pitch_motor.given_current;
#else
    pitch_can_set_current = gimbal_control_1.gimbal_pitch_motor.given_current;
#endif
...
static void gimbal_init(gimbal_control_t *init)
{
  ...
  init->gimbal_INS_gyro_point = get_gyro_data_point();
...
static void gimbal_init_1(gimbal_control_t *init)
{
  ...
  // 云台yaw电机速度pid初始化
  PID_init(&init->gimbal_yaw_motor.gimbal_motor_gyro_pid, PID_POSITION, Yaw_speed_pid, YAW_SPEED_PID_MAX_OUT, YAW_SPEED_PID_MAX_IOUT);
  init->gimbal_yaw_motor.offset_ecd=4006;
...
static void gimbal_feedback_update(gimbal_control_t *feedback_update)
{
  ...
  // 获取绝对角度absolute_angle：
  // 以惯导数据指针为基准，加上惯导中pitch数据所在位置的偏置量，即可寻址到pitch轴数据，将其值赋予绝对角度
  feedback_update->gimbal_pitch_motor.absolute_angle = *(feedback_update->gimbal_INS_angle_point + INS_PITCH_ADDRESS_OFFSET);
  // 以惯导数据指针为基准，加上惯导中yaw数据所在位置的偏置量，即可寻址到yaw轴数据，将其值赋予绝对角度
  feedback_update->gimbal_yaw_motor.absolute_angle = *(feedback_update->gimbal_INS_angle_point + INS_YAW_ADDRESS_OFFSET);

  // 获取相对角度relative_angle：
  // 如果pitch电机反转，则相对角度取负值，否则相对角度取正值
#if PITCH_TURN
  feedback_update->gimbal_pitch_motor.relative_angle = -motor_ecd_to_angle_change(
    feedback_update->gimbal_pitch_motor.gimbal_motor_measure->ecd, feedback_update->gimbal_pitch_motor.offset_ecd);
#else
  feedback_update->gimbal_pitch_motor.relative_angle = motor_ecd_to_angle_change(
    feedback_update->gimbal_pitch_motor.gimbal_motor_measure->ecd, feedback_update->gimbal_pitch_motor.offset_ecd);
#endif
  // 如果yaw电机反转，则相对角度取负值，否则相对角度取正值
#if YAW_TURN
  feedback_update->gimbal_yaw_motor.relative_angle = -motor_ecd_to_angle_change(
    feedback_update->gimbal_yaw_motor.gimbal_motor_measure->ecd, feedback_update->gimbal_yaw_motor.offset_ecd);
#else
  feedback_update->gimbal_yaw_motor.relative_angle = motor_ecd_to_angle_change(
    feedback_update->gimbal_yaw_motor.gimbal_motor_measure->ecd + 4096, feedback_update->gimbal_yaw_motor.offset_ecd);
#endif

  // 获取陀螺仪数据：
  // 获取pitch轴角速度数据，以惯导数据指针为基准，加上陀螺仪Y轴数据所在位置的偏置量，即可寻址到陀螺仪Y轴数据，将其值赋予pitch轴的gyro角速度
  feedback_update->gimbal_pitch_motor.motor_gyro = *(feedback_update->gimbal_INS_gyro_point + INS_GYRO_Y_ADDRESS_OFFSET);
  // 获取yaw轴角速度数据，用pitch轴相对角度的cos值乘以惯导Z轴陀螺仪数据，再减去pitch轴相对角度的sin值与惯导X轴陀螺仪数据之积，即可得到yaw轴的gyro角速度
  feedback_update->gimbal_yaw_motor.motor_gyro = arm_cos_f32(feedback_update->gimbal_pitch_motor.relative_angle) * 
  (*(feedback_update->gimbal_INS_gyro_point + INS_GYRO_Z_ADDRESS_OFFSET)) - 
  arm_sin_f32(feedback_update->gimbal_pitch_motor.relative_angle) * 
  (*(feedback_update->gimbal_INS_gyro_point + INS_GYRO_X_ADDRESS_OFFSET));

  // 读取GPIOA端口的GPIO_PIN_0引脚的状态，并将结果赋值给变量KEY_1
  KEY_1 = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);
  // 处理按键事件
  KEY_Process();
}
...
static void gimbal_mode_change_control_transit(gimbal_control_t *gimbal_mode_change)
{
  ...
  // yaw电机从其他模式切换到GIMBAL_MOTOR_RAW原始模式
  if (gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_RAW && gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_RAW)
  {
    gimbal_mode_change->gimbal_yaw_motor.raw_cmd_current = gimbal_mode_change->gimbal_yaw_motor.current_set = gimbal_mode_change->gimbal_yaw_motor.given_current;
  }
  // yaw电机从其他模式切换到GIMBAL_MOTOR_GYRO陀螺仪模式
  ...
  // yaw电机从其他模式切换到GIMBAL_MOTOR_ENCODE编码器模式
  ...
  // 将yaw电机的last mode同步为当前mode
  gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode = gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode;

  // pitch电机从其他模式切换到GIMBAL_MOTOR_RAW原始模式
  ...
  // pitch电机从其他模式切换到GIMBAL_MOTOR_GYRO陀螺仪模式
  ...
  // pitch电机从其他模式切换到GIMBAL_MOTOR_ENCODE编码器模式
...
static void gimbal_set_control(gimbal_control_t *set_control)
{
  ...
  gimbal_control_1.gimbal_yaw_motor.max_relative_angle = max_relative_angle_yaw;
  gimbal_control_1.gimbal_yaw_motor.min_relative_angle = min_relative_angle_yaw;
  gimbal_control_1.gimbal_pitch_motor.max_relative_angle = max_relative_angle_pitch;
  gimbal_control_1.gimbal_pitch_motor.min_relative_angle = min_relative_angle_pitch;
  fp32 add_yaw_angle = 0.0f;
  fp32 add_pitch_angle = 0.0f;

  gimbal_behaviour_control_set(&add_yaw_angle, &add_pitch_angle, set_control);
  
  // 根据云台 yaw（偏航）电机的模式，设置其控制指令
  if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_RAW)
  {
    // 在原始模式下，直接设置偏航电机的原始指令电流
    set_control->gimbal_yaw_motor.raw_cmd_current = add_yaw_angle;
  }
  else if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO)
  {
    // 在陀螺仪模式下，使用绝对角度限制来控制偏航电机
    gimbal_absolute_angle_limit(&set_control->gimbal_yaw_motor, add_yaw_angle);
  }
  else if (set_control->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
  {
    // 在编码器模式下，使用相对角度限制来控制偏航电机
    gimbal_relative_angle_limit(&set_control->gimbal_yaw_motor, add_yaw_angle);
  }
  
  // 根据云台 pitch（俯仰）电机的模式，设置其控制指令
  if (set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_RAW)
  {
    // 在原始模式下，直接设置俯仰电机的原始指令电流
  ...
  {
    // 在陀螺仪模式下，使用绝对角度限制来控制俯仰电机
    gimbal_absolute_angle_limit_1(&set_control->gimbal_pitch_motor, add_pitch_angle);
  }
  else if (set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
  {
    // 在编码器模式下，使用相对角度限制来控制俯仰电机
    gimbal_relative_angle_limit_1(&set_control->gimbal_pitch_motor, add_pitch_angle);
  }
  yaw_add = add_yaw_angle;
  pitch_add = add_pitch_angle;
}
